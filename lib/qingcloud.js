...
export default class QingcloudAPI {
  ...
  async fetchBackends(lbconfig: LBConfig): Promise<Array<Object>> {
    const response = await this.request('DescribeLoadBalancerBackends', {
      loadbalancer_backend_id: lbconfig.listenerID
    });
  ...
  async syncBackends(lbconfig: LBConfig, newBackends: Array<Backend>): Promise<void> {
    ...
    const updateBalancer = backendsToRemove.length || backendsToAdd.length;
    ...
    if (updateBalancer) {
      console.log('updateLoadBalancers');
      await this.updateLoadBalancers([lbconfig]);
    }
  }
